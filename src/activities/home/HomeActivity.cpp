...
int HomeActivity::getMenuItemCount() const {
  int count = 3;  // Library, File Browser, Settings
...
void HomeActivity::loadRecentCovers(int coverHeight) {
  ...
  if (!coverGridUi && !recentBooks.empty()) {
    recentBooks.front().progressPercent = loadBookProgress(recentBooks.front().path);
    requestUpdate();
  }
}
...
void HomeActivity::render(RenderLock&&) {
  ...
  std::vector<const char*> menuItems = {tr(STR_LIBRARY), tr(STR_BROWSE_FILES), tr(STR_SETTINGS_TITLE)};
  // Bookshelf for the on-device library, Library for the OPDS catalogue below —
  // the two entries need distinct icons.
  std::vector<UIIcon> menuIcons = {Library, Folder, Settings};
