...
class OpdsSettingsActivity final : public UiListActivity {
 ...
 private:
  ...
  // Row storage: at most 6 rows (Name/URL/Username/Password/Download folder +
  // Delete, see BASE_ITEMS in the .cpp), so a fixed-capacity array avoids any
  // heap allocation for the row list. Labels are set once in the constructor
