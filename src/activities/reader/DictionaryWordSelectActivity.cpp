...
bool DictionaryWordSelectActivity::wasPressedVisual(const VisualDir dir) const {
  ...
  const bool horizontal = dir == VisualDir::Left || dir == VisualDir::Right;
  const bool repeat = horizontal && mappedInput.getHeldTime() >= WORD_REPEAT_START_MS &&
                      millis() - lastHorizontalMoveTime >= WORD_REPEAT_INTERVAL_MS;
  return mappedInput.wasPressed(button) || (repeat && mappedInput.isPressed(button));
}
