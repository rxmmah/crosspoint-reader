...
constexpr size_t ITEM_INDEX_BYTES_PER_ENTRY_ESTIMATE = 96;
}  // namespace
...
bool ContentOpfParser::setup() {
  ...
  if (xmlSize >= LARGE_SPINE_THRESHOLD * ITEM_INDEX_BYTES_PER_ENTRY_ESTIMATE) {
    itemIndex.reserve((xmlSize / ITEM_INDEX_BYTES_PER_ENTRY_ESTIMATE) + 1);
  }
  return true;
}

void ContentOpfParser::failParse(const char* reason) {
  if (parseFailed) {
    return;
  }
  parseFailed = true;
